...
void CDlgShowArray::OnButtonSave()
{
	...
	{
		// 保存成24位彩色BMP图像
		SaveAsBmp((char*)(LPCTSTR)dlg.GetPathName(), _lpBuf, _width, _height, 24,
			NULL);
	}
}
...
void CDlgShowArray::InitComboType() {
	...
	_comboType.InsertString(ALL, _T("全部"));
	_comboType.InsertString(RED, _T("红色通道"));
	_comboType.InsertString(GREEN, _T("绿色通道"));
	_comboType.InsertString(BLUE, _T("蓝色通道"));
